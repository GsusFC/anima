...
function buildUnifiedTransitionChain(validImages, transitions, frameDurations, duration, complexFilter) {
  ...
  console.log(`🎞️ [INDEX] Starting transition chain for ${validImages.length} images`);
  console.log(`🎞️ [INDEX] Frame durations:`, frameDurations.map((d, i) => `${i}: ${d}ms`));
  ...
  for (let i = 0; i < validImages.length - 1; i++) {
    ...
    console.log(`🎞️ [INDEX] Frame ${i}->${i+1}: duration=${currentFrameDuration}s, total=${totalVideoTime}s, transition=${transitionDuration}s, offset=${offset}s`);
    console.log(`🎞️ [INDEX] XFade filter: ${xfadeFilter}`);
